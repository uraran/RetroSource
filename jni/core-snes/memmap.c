...
//	Memory.ROM  = (uint8 *) malloc(MAX_ROM_SIZE + 0x200 + 0x8000);
	Memory.FillRAM = (uint8 *) malloc(0x8000);
	...
	if (!Memory.RAM || !Memory.SRAM || !Memory.VRAM || /*!Memory.ROM || */ !Memory.FillRAM ||
...
bool8 LoadROM (const void *buffer, int romSize)
{
...
again:
	...
	totalFileSize = romSize;
	Memory.ROM = buffer;

	// set pointers previously set inside of Init()
	Memory.C4RAM   = Memory.ROM + 0x400000 + 8192 * 8; /* C4 */
	Memory.OBC1RAM = Memory.ROM + 0x400000; /* OBC1 */
	Memory.BIOSROM = Memory.ROM + 0x300000; /* BS */
	Memory.BSRAM   = Memory.ROM + 0x400000; /* BS */

	// NOTE: this can lead to invalid accesses in the case of mmap'd cart, so we must force normal file loading for SFX games
	SuperFX.pvRom       = (uint8 *) Memory.ROM;
...
bool8 LoadSRAM (const char *filename)
{
	...
	if (size)
	{
		...
		if (file)
		{
			len = fread((char *) Memory.SRAM, 1, 0x20000, file);
			fclose(file);
			if (len - size == 512)
...
#if defined(__LIBRETRO__) && defined(DEBUG)
									...
									if (strncmp(Memory.ROMName, "SOUND NOVEL-TCOOL", 17) == 0 ||
												...
												strncmp(Memory.ROMName, "RPG-TCOOL 2", 11) == 0)
										{
											...
											if (strncmp(Memory.ROMName, "ADD-ON BASE CASSETE", 19) == 0)
											{
												...
												if (strncmp(Memory.ROMName, "\x5C\x04\x80\x80icart menu", 14) == 0)
												{
													LOGI("mapping for 4in1\n");
													SNESGameFixes.Super4in1 = 1;
													S9xSetMapper(0, 0);
												}
												else
												{
...
/*
S9xSetMapper: 82, FF82 - mazezam (lorom, bank 0x02 = off 512kb)
S9xSetMapper: C8, FFC8 - n.warp daisakusen (hirom, bank 0x08 = off 2mb)
S9xSetMapper: 84, FF84 - skipp and friends (lorom, bank 0x04 = off 1mb)
S9xSetMapper: 81, FF81 - uwol quest for money (lorom, bank 0x01 = off 256kb)
*/
void	S9xSetMapper(uint8 Byte, uint16 Address)
{
	if(SNESGameFixes.Super4in1 == 0)
	{
		LOGE("mapper not supported for this title\n");
		return;
	}
	LOGI("S9xSetMapper: %X, %X\n", Byte, Address);

	int offset = (Byte & 0x1F) * 0x40000;
	if(Byte & 0x40)
	{
		LOGI("remapping hirom with offset = 0x%X\n", offset);
		MAP_INITIALIZE();
		MAP_SYSTEM();
		MAP_HIROM_OFFSET(0x00, 0x3f, 0x8000, 0xffff, /*Memory.CalculatedSize - */0x400000, offset);
		MAP_HIROM_OFFSET(0x40, 0x7f, 0x0000, 0xffff, /*Memory.CalculatedSize - */0x400000, offset);
		MAP_HIROM_OFFSET(0x80, 0xbf, 0x8000, 0xffff, 0x400000, offset);
		MAP_HIROM_OFFSET(0xc0, 0xff, 0x0000, 0xffff, 0x400000, offset);
		MAP_HIROMSRAM(); \
		MAP_WRAM(); \
		map_WriteProtectROM();
		return;
	...
	{
		LOGI("remapping lorom with offset = 0x%X\n", offset);
		MAP_INITIALIZE();
		MAP_SYSTEM();
		MAP_LOROM_OFFSET(0x00, 0x3f, 0x8000, 0xffff, /*Memory.CalculatedSize - */0x400000, offset);
		MAP_LOROM_OFFSET(0x40, 0x7f, 0x0000, 0xffff, /*Memory.CalculatedSize - */0x400000, offset);
		MAP_LOROM_OFFSET(0x80, 0xbf, 0x8000, 0xffff, 0x400000, offset);
		MAP_LOROM_OFFSET(0xc0, 0xff, 0x0000, 0xffff, 0x400000, offset);
		MAP_LOROMSRAM();
		MAP_WRAM();
		map_WriteProtectROM();
	}
	Memory.WriteMap[0x1000-1] = (uint8 *) MAP_MAPPER;
	Settings.LastMapperValue = Byte;
}
