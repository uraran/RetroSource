...
static void getrominfo(char *romheader)
{
  ...
  else
  {
    ...
    if (system_hw == SYSTEM_PBC || system_hw == SYSTEM_GAMEGEAR) return;
...
#endif
    rominfo.realchecksum = 0;//getchecksum(((uint8 *) cart.rom) + 0x200, cart.romsize - 0x200);
...
static void deinterleave_block(uint8 * src)
{
  ...
  * Load a new ROM file.
  ***************************************************************************/
int load_rom(void *buf, int rom_size, int system_type)
{
  int i, size;
  ...
  // retron cart mmap has bytes swapped, so we need to reverse for the header parsing
  if (system_hw != SYSTEM_PBC && system_hw != SYSTEM_GAMEGEAR && rom_size)
  {
	uint8_t hdr[1024];
  ...
  if (system_hw != SYSTEM_PBC && system_hw != SYSTEM_GAMEGEAR)
  {
	// only byte-swap when reading from a file, retron cart mmap will have the bytes swapped in advance
	if(!rom_size)
	{
#ifdef LSB_FIRST
    /* Byteswap ROM */
...
/*
	...
	// for RetroFreak we need to give Japan the highest priority
    if (country & 1) region_code = REGION_JAPAN_NTSC;
    else if (country & 4) region_code = REGION_USA;
    else if (country & 8) region_code = REGION_EUROPE;
    else if (country & 2) region_code = REGION_JAPAN_PAL;
    else region_code = REGION_JAPAN_NTSC;
