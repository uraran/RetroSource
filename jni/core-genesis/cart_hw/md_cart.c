  ...
  if (sram.on)
  {
    ...
    else
    {
      ...
      if (cart.romsize <= sram.start)
      {
        ...
        m68k_memory_map[sram.start >> 16].read8   = sram_read_byte;
        m68k_memory_map[sram.start >> 16].read16  = sram_read_word;
        m68k_memory_map[sram.start >> 16].write8  = sram_write_byte;
        m68k_memory_map[sram.start >> 16].write16 = sram_write_word;
...
int md_cart_context_load(uint8 *state)
{
  ...
  for (i=0; i<0x40; i++)
  {
    ...
    if (offset == 0xff)
    {
      ...
      m68k_memory_map[i].read8    = sram_read_byte;
      m68k_memory_map[i].read16   = sram_read_word;
      m68k_memory_map[i].write8   = sram_write_byte;
      m68k_memory_map[i].write16  = sram_write_word;
  ...
  if (data & 1)
  {
    if (sram.on)
    {
      /* Backup RAM mapped to $200000-$20ffff (normally mirrored up to $3fffff but this breaks Sonic Megamix and no game need it) */
        ...
        m68k_memory_map[0x20].read8   = sram_read_byte;
        m68k_memory_map[0x20].read16  = sram_read_word;
        ...
        {
          m68k_memory_map[0x20].write8  = sram_write_byte;
          m68k_memory_map[0x20].write16 = sram_write_word;
