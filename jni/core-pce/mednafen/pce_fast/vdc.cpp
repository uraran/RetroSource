   ...
   else if(vdc->display_counter >= (VDS + VSW) && vdc->display_counter < (VDS + VSW + VDW + 1))
   {
    ...
    if(frame_counter >= 14 && frame_counter < (14 + 242))
    {
     ...
     if(SHOULD_DRAW)
     {
      ...
      //	MDFN_DispMessage("soffset=%d, toffset=%d, width=%d, CR=%X", source_offset, target_offset, width, vdc->CR & 0xC0);
...
//  frame_counter = (frame_counter + 1) % (vce.lc263 ? 263 : 262);
	...
	// HACK: force 263 lines in order to keep the frame rate the same for every game, and thus the same audio output rate
	// TODO - does this have any negative side effects?
	frame_counter = (frame_counter + 1) % 263;
...
static bool LoadCustomPalette(const char *path)
{
 ...
 else if((CustomColorMap = (uint8*)MDFN_malloc(1024 * 3, _("custom color map"))))
 {
  ...
  length_read = fread(CustomColorMap, 1024 * 3, 1, gp);
