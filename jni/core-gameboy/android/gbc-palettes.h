...
#define PACK15_1(rgb24) (TO5BIT((rgb24) >> 16 & 0xFF) << 0 | TO5BIT((rgb24) >> 8 & 0xFF) << 5 | TO5BIT((rgb24) & 0xFF) << 10)
...
static const unsigned short * findGbcDirPal(const char *const title) {
	for(int i = 0; i < sizeof(gbcDirPalettes)/sizeof(GbcPaletteEntry); i++)
		if(!strcmp(gbcDirPalettes[i].title, title))
			return gbcDirPalettes[i].p;
	return NULL;
}

static const unsigned short * findGbcTitlePal(const char *const title) {
	for(int i = 0; i < sizeof(gbcTitlePalettes)/sizeof(GbcPaletteEntry); i++)
		if(!strcmp(gbcTitlePalettes[i].title, title))
			return gbcTitlePalettes[i].p;
	return NULL;
}
